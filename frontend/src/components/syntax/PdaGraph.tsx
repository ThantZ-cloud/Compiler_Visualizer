    ...
    <div className="w-full max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4">
        ...
        <div className="border border-[var(--color-border-bright)] rounded-lg bg-[var(--color-card)] p-3">
          <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} className="w-full h-auto" style={{ fontFamily: "'Consolas', 'Monaco', monospace" }}>
            <defs>
            ...
            {PDA_TRANSITIONS.map((tr, i) => {
              ...
              return (
                <g key={i} opacity={active ? 1 : 0.55} style={{ color }}>
                  <path d={d} fill="none" stroke={color} strokeWidth="1.5" markerEnd="url(#pda-arrow)" />
                  <text x={lx} y={ly - 6} fill="var(--color-text-muted)" fontSize="8" textAnchor="middle">
                    {tr.label}
                  </text>
                </g>
              );
            ...
            {PDA_STATES.map(state => {
              ...
              const radius = isActiveState ? 22 : 18;
              return (
                <g key={state.id} opacity={isActiveState ? 1 : 0.65}>
                  {isActiveState && (
                    <circle cx={state.x} cy={state.y} r={radius + 6} fill="none" stroke="var(--color-neon)" strokeOpacity="0.35">
                  ...
                  <circle
                    ...
                    strokeWidth={isActiveState ? 2 : 1.2}
                  />
                  {state.isStart && <path d={`M ${state.x - radius} ${state.y} h ${radius - 2}`} stroke="var(--color-neon)" fill="none" markerEnd="url(#pda-arrow)" />}
                  {state.isAccept && (
                    <circle cx={state.x} cy={state.y} r={radius - 5} fill="none" stroke="var(--color-amber)" strokeWidth="1.2" />
                  )}
                  <text x={state.x} y={state.y - radius - 8} fill={isActiveState ? 'var(--color-neon)' : 'var(--color-text)'} fontSize="10" fontWeight="bold" textAnchor="middle" letterSpacing="1">
                    {state.label}
                  </text>
                  <text x={state.x} y={state.y + 4} fill="var(--color-text-muted)" fontSize="7" textAnchor="middle">
                    {state.description}
                  </text>
                </g>
              );
