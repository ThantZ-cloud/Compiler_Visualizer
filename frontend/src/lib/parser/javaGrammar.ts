...
export const GRAMMAR_RULES: GrammarRule[] = [
  ...
  { id: 'G21', lhs: 'IfStmt', rhs: ["'if'", "'('", 'Expr', "')'", 'Stmt', "'else'?", 'Stmt?'], description: '`if (cond) ... else ...` — classic dangling-else ambiguity; Java binds else to the innermost unclosed if (shift on else). Table shows EBNF shorthand; the full grammar rewrites it via a WithElse rule.' },
  ...
  { id: 'G28', lhs: 'Expr', rhs: ['AssignExpr'], description: 'Assignment expressions (`x = ...`) — right-associative via recursive RHS.' },
  { id: 'G29', lhs: 'AssignExpr', rhs: ['Expr', 'op', 'Expr'], description: '`lhs op rhs` — `=` is right-assoc; tree shape shows it.' },
  { id: 'G30', lhs: 'Expr', rhs: ['BinaryExpr'], description: 'Binary arithmetic / logical expressions. Precedence is encoded by stratification in a full grammar (Expr→Term→Factor); this table collapses tiers for brevity — the AST tree shape (a+b*c → a+(b*c)) still demonstrates precedence.' },
  { id: 'G31', lhs: 'BinaryExpr', rhs: ['Expr', 'op', 'Expr'], description: '`a op b` — `* / %` bind tighter than `+ -` because the subtree for `b*c` is built first.' },
