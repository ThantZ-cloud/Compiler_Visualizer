...
const BytecodePanel: React.FC = () => {
  ...
  const handlePlayOnePhase = useCallback(() => {
    ...
    const stepDelays = [...BASE_STEP_DELAYS];
    if (firstMethod && step===1) { const traceSteps = simulateExecution(firstMethod.instructions, firstMethod.maxLocals).steps.length; if (traceSteps > 0) { const msPerStep = Math.max(120, Math.min(600, Math.floor(10000 / traceSteps))); stepDelays[1] = traceSteps * msPerStep + 1500; } }
  ...
  if (!hasBackend) return <div className="flex flex-col items-center justify-center h-full text-[var(--color-text-muted)] text-center gap-3"><Binary size={48} className="text-[var(--color-neon)] opacity-30" /><div className="font-mono text-sm">{t('bytecode.noBytecode')}</div></div>;
